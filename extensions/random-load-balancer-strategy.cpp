...
*/
  ...
  do {
  	//std::cout << nexthops.size();  // nexthops.size() = 2
    boost::random::uniform_int_distribution<> dist(0, nexthops.size() - 1);
    const size_t randomIndex = dist(m_randomGenerator);
    ...
    std::cout << "Once selected face:" << selected->getFace() << " | "; 
  } while (!canForwardToNextHop(inFace, pitEntry, *selected));
  std::cout << "Final selected face:" << selected->getFace() << std::endl;
  this->sendInterest(pitEntry, selected->getFace(), interest);
  /*
  	Only two face below:
	face:netdev://[00:00:00:00:00:07]
	face:netdev://[00:00:00:00:00:05]
  */
}
